-- Migration: 000016_fix_fk_cascades
-- Purpose: Enable ON UPDATE CASCADE for all foreign keys referencing users(id)
-- This allows updating user.id (e.g. when syncing from Supabase) to propagate to child tables automatically.
...
ALTER TABLE account_verifications ADD CONSTRAINT account_verifications_verified_by_fkey 
    FOREIGN KEY (verified_by) REFERENCES users(id) ON UPDATE CASCADE;
...
ALTER TABLE applications ADD CONSTRAINT applications_candidate_user_id_fkey 
    FOREIGN KEY (candidate_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE;
